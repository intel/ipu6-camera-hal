...
int FileSource::allocateSourceBuffer()
{
    ...
    if (mInjectionWay == USING_CONFIG_FILE) {
        ...
        CheckAndLogError((mInjectionWay < USING_FRAME_FILE ||
                         mInjectionWay >= UNKNOWN_INJECTED_WAY), BAD_VALUE, "Invalid Injected Way");
    }
...
void FileSource::fillFrameBuffer(shared_ptr<CameraBuffer>& buffer)
{
    ...
    CheckAndLogError(fileName.empty(), VOID_VALUE, "Invalid frame file.");
    LOG2("Frame sequence:%ld uses frame file:%s, buffer %p", mSequence, fileName.c_str(),
...
{
    LOGXML("Common fps:%f, frame dir:%s", mCommon.mFps, mCommon.mFrameDir.c_str());
    ...
    for (auto& item : mConfigs) {
        ...
        LOGXML("Sensor:%s, fps:%f frame dir:%s", item.first.c_str(),
...
{
    LOG1("%s: FileSourceFromDir is destroyed.", __func__);
}

int FileSourceFromDir::getInjectionFiles()
{
    LOG1("Enter %s for debugging.", __func__);

    struct dirent* fileDirent = nullptr;
    struct stat statBuf;

    DIR* dirInfo = opendir(mInjectionPath.c_str());
    CheckAndLogError(!dirInfo, BAD_VALUE, "Invalid injection path: %s.", mInjectionPath.c_str());
    ...
    CheckAndLogError(!mInjectionFiles.size(), BAD_VALUE, "No Injection files");
    sort(mInjectionFiles.begin(), mInjectionFiles.end());

    LOG1("Exit %s for debugging.", __func__);
...
int FileSourceFromDir::getInjectionFileInfo(map<int, string> *frameFilesInfo)
{
    ...
    int ret = getInjectionFiles();
    CheckAndLogError(ret != OK, BAD_VALUE, "getInjectionFiles failed");
...
string FileSourceFromDir::getFrameFile(const map<int, string> &frameFilesInfo, long sequence)
{
    LOG1("Enter %s for debugging.", __func__);
