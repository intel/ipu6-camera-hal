...
int CsiMetaDevice::decodeMetaData(EventDataMeta& metaData, const shared_ptr<CameraBuffer> &camBuffer)
{
    ...
    if (mEmbeddedMetaData.planesNum > 0) {
        ...
        LOG2("%s: sequence %ld, offset %d/%d, value is %d", __func__, metaData.sequence,
...
int CsiMetaDevice::allocCsiMetaBuffers()
{
    ...
    for (int i=0; i<bufNum; i++) {
        shared_ptr<CameraBuffer> camBuffer = std::make_shared<CameraBuffer>(mCameraId, BUFFER_USAGE_METADATA,
                                               V4L2_MEMORY_MMAP, 0, i,V4L2_FMT_IPU_ISYS_META);
