...
private:
    ...
    enum {
        USING_FRAME_FILE = 0,   // If mInjectedFile is a frame file name, it means we're using frame file.
        USING_CONFIG_FILE,      // If mInjectedFile ends with ".xml", it means we're using config file.
        USING_INJECTION_PATH,   // If mInjectedFile is a directory, it means we're using injection path.
...
class FileSourceProfile {
public:
    FileSourceProfile(std::string configFile);
    ~FileSourceProfile() {}

    float getFps(int cameraId);
    std::string getFrameFile(int cameraId, long sequence);
 ...
 *
 * It's used to parse file source from injection file directory, and provide such frame file name etc
 * information for FileSource to use.
 */
class FileSourceFromDir {
 public:
    FileSourceFromDir(const std::string& injectionPath);
    ~FileSourceFromDir();

    int getInjectionFileInfo(std::map<int, std::string> *frameFilesInfo);
    std::string getFrameFile(const std::map<int, std::string> &frameFilesInfo, long sequence);

 private:
    int getInjectionFiles();
