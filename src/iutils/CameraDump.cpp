...
bool DumpThread::threadLoop() {
    ...
    LOGD("Process %d opened the fifo, fd %d\n", getpid(), pipe_fd);
    ...
    if (pipe_fd != -1) {
        ...
        (void)close(pipe_fd);
        LOGD("Process %d closed the fifo\n", getpid());
    }
    ...
    if (!strncmp(PROP_CAMERA_HAL_DUMP, buffer, strlen(PROP_CAMERA_HAL_DUMP))
        ...
        LOGD("set %s=%s\n", PROP_CAMERA_HAL_DUMP_FORMAT, equal + 1);
        ...
        LOGD("set %s=%s\n", PROP_CAMERA_HAL_DUMP_PATH, equal + 1);
        ...
        LOGD("set %s=%s\n", PROP_CAMERA_HAL_DUMP_SKIP_NUM, equal + 1);
        ...
        LOGD("set %s=%s\n", PROP_CAMERA_HAL_DUMP_RANGE, equal + 1);
